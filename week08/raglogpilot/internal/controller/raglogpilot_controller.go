...
func (r *RagLogPilotReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// 检查是否有 ChatId
	if ragLogPilot.Status.ChatId == "" {
		chatId, err := r.createNewChat(ragLogPilot)
		if err != nil {
			logger.Error(err, "failed to create new chat")
			return ctrl.Result{}, err
		}

		// 保存 ConversationId 到 Status
		ragLogPilot.Status.ChatId = chatId
		if err := r.Status().Update(ctx, &ragLogPilot); err != nil {
			logger.Error(err, "failed to update status with ChatId")
			return ctrl.Result{}, err
		}
	}

	// 检查是否有 SessionId
	if ragLogPilot.Status.SessionId == "" {
		sessionId, err := r.createNewSession(ragLogPilot)
		if err != nil {
			logger.Error(err, "failed to create new session")
			return ctrl.Result{}, err
		}

		// 保存 ConversationId 到 Status
		ragLogPilot.Status.SessionId = sessionId
		if err := r.Status().Update(ctx, &ragLogPilot); err != nil {
			logger.Error(err, "failed to update status with SessionId")
			return ctrl.Result{}, err
		}
	}
	...
	for _, pod := range pods.Items {
		...
		if len(errorLog) > 0 {
			...
			//err = r.sendFeishuAlert("https://open.feishu.cn/open-apis/bot/v2/hook/xxxx-xxxx-xxxx-xxxx", answer)
			err = r.sendFeishuAlert(ragLogPilot.Spec.FeishuBotWebhook, answer)
			if err != nil {
				fmt.Println(err)
			}
...
// 创建新的 conversation
func (r *RagLogPilotReconciler) createNewChat(ragLogPilot logv1.RagLogPilot) (string, error) {
	// 创建请求体数据
	body := RequestBody{
		DatasetIDs: []string{ragLogPilot.Spec.DatasetId},
		Name:       fmt.Sprintf("chat_%d", time.Now().UnixNano()),
		Language:   "Chinese",
		LLM:        LLM{ModelName: "qwen2:7b"},
	}
	// 将结构体序列化为 JSON
	...
	req, err := http.NewRequest("POST", fmt.Sprintf("%s/chats", ragLogPilot.Spec.RagFlowEndpoint), bytes.NewBuffer(jsonData))
...
func (r *RagLogPilotReconciler) createNewSession(ragLogPilot logv1.RagLogPilot) (string, error) {
	// 创建请求体数据
	...
	req, err := http.NewRequest("POST", fmt.Sprintf("%s/chats/%s/sessions", ragLogPilot.Spec.RagFlowEndpoint, ragLogPilot.Status.ChatId), bytes.NewBuffer(body))
	//req, err := http.NewRequest("GET", fmt.Sprintf("%s/new_conversation?user_id=%s", ragLogPilot.Spec.RagFlowEndpoint, uuid.NewString()), nil)
	if err != nil {
		return "", err
	}
...
func (r *RagLogPilotReconciler) queryRagSystem(podLog string, ragLogPilot logv1.RagLogPilot) (string, error) {
	...
	req, err := http.NewRequest("POST", fmt.Sprintf("%s/chats/%s/completions", ragLogPilot.Spec.RagFlowEndpoint, ragLogPilot.Status.ChatId), bytes.NewBuffer(body))
	if err != nil {
		return "", err
	}
	req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", ragLogPilot.Spec.RagFlowToken))
